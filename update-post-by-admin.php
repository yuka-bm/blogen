    ...
    <div class="container text-center mt-5 w-50">
        <form action="#" method="POST">
            ...
            <div class="input-group mt-4">
                ...
                <select name="account_id" id="account_id" class="form-select border-1 border-dark border-top-0 border-start-0 border-end-0 rounded-0" required>
                    <option value="" hidden>Select Author</option>
                    <?php
                        $categories = getAllAuthors();
                        while ($category = $categories->fetch_assoc()) {
                            if ($category['account_id'] == $post['account_id']) {
                                echo "<option value=" . $category['account_id'] . " selected>" . $category['username'] . "</option>";
                            }
                            else  {
